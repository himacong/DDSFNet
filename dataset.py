...
class M3FD_detect(Dataset):
    def __init__(self, cfg: ConfigDict, mode: Literal['train', 'val', 'test']):
        super().__init__()
        self.cfg = cfg
        self.mode = mode
        self.img_list = os.listdir(os.path.join(cfg.dataset_root, 'ir'))
...
class M3FD(Dataset):
    def __init__(self, cfg: ConfigDict, mode: Literal['train', 'val', 'test']):
        ...
        if self.mode == 'train' and cfg.have_seg_label == True:
            ...
            self.mask_path = Path(Path(self.cfg.dataset_root) / 'labels')
        if self.mode == 'test':
            check_mask(Path(cfg.dataset_root) / 'test', self.img_list, cfg)
            self.mask_path = Path(Path(self.cfg.dataset_root) / self.mode / 'mask')
...
if __name__ == '__main__':
    ...
    config = yaml.safe_load(open('./configs/cfg22.yaml'))
