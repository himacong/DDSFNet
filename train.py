    ...
    for epoch in range(cfg.start_epoch, cfg.num_epochs):
        ...
        for data_ir, data_vi, mask, _ in iter:
            ...
            for m in module_list:
                m.train()
            fus_data= fuse_net(data_ir, data_vi,mask)
            # conten_loss
            ...
            total_loss = cfg.coeff_content * content_loss + cfg.coeff_ssim * ssim_loss 
        ...
        logging.info(
            f'Epoch {epoch + 1}/{cfg.num_epochs}, lr:{optimizer.param_groups[0]["lr"]}, total_loss: {total_loss_meter.avg}, content_loss: {content_loss_meter.avg}, ssim_loss: {ssim_loss_meter.avg}'
        )
